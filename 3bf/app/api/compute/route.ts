...
export async function POST(req: Request) {
  try {
    ...
    const workerUrl = process.env.NEXT_PUBLIC_3BF_WORKER_URL || "http://127.0.0.1:8005";

    // Intentar conectar con el 3BF Worker Python
    try {
      const workerRes = await fetch(`${workerUrl}/compute`, {
        method: "POST",
      ...
      console.warn("⚠️ 3BF Worker Python offline. Usando fallback de cálculo en API Route.");
    }
