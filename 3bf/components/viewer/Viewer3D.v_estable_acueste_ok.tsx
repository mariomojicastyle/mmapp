...
function SubbloqueSingleTooltip({
  sub,
  sIdx,
  pasoId,
  furnitureGroup,
  ...
  const [centerPos, setCenterPos] = React.useState<[number, number, number] | null>(null);
  ...
  useFrame(() => {
    const piezasDelSub = [...(sub.piezas || []), ...(sub.herrajes || [])];
    ...
    furnitureGroup.traverse((obj) => {
      if ((obj as THREE.Mesh).isMesh && obj.visible) {
        ...
        const match = piezasDelSub.some((target) => {
          ...
          const tClean = target.replace(/^RH_OUT:\s*/i, "").trim();
          const tPM = extraerPiezaMadre(tClean);
          return (
            target === cleanName ||
            target === rawName ||
            target === instKey ||
            tClean === cleanName ||
            tPM === pm ||
            (instKey && (target === instKey || tClean === instKey))
          );
        });
...
function SubbloquesTooltipsBillboard({ furnitureGroup }: { furnitureGroup: THREE.Group | null }) {
