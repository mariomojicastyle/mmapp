...
export function CapaPiezaEsperaItem({
  ...
  return (
    ...
    >
      {/* Fila 1: Identificación, Estado y Posicionamiento */}
      <div className="flex items-center justify-between gap-2">
        {/* Agarrador de Arrastre Exclusivo o Corona Master */}
        {!esMaster ? (
          <div
        ...
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          ...
          {/* 👁️ Segundo en que la capa/pieza aparece a escala real (por defecto 0s) */}
          <div
            onClick={(e) => e.stopPropagation()}
            title={`👁️ Aparición en escena: segundo en que ${p.nombrePieza} aparece visible (0s = desde el inicio del paso)`}
            className="inline-flex items-center gap-0.5 bg-white/90 dark:bg-slate-900/90 border border-slate-300/80 dark:border-slate-600/80 rounded-full px-1.5 py-0 shadow-2xs shrink-0 hover:border-cyan-500 transition cursor-text ml-0.5"
          >
          ...
          <div
            ...
            className="inline-flex items-center gap-0.5 bg-white/90 dark:bg-slate-900/90 border border-slate-300/80 dark:border-slate-600/80 rounded-full px-1.5 py-0 shadow-2xs shrink-0 hover:border-amber-500 transition cursor-text ml-0.5"
          >
        ...
        {/* 🎯 BOTÓN INTERMEDIO: POSICIONAR EN ESCENARIO (Mouse / Mira) */}
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
        ...
        {/* Entradas numéricas X e Y (Offset en cm en el piso) */}
        <div className="flex items-center gap-1.5 shrink-0 font-mono text-[10px]">
          ...
          {/* ↺ Botón de Reset al Origen */}
          <button
            type="button"
            onClick={() => onResetOffset(p.nombrePieza)}
            title={`Restablecer ${p.nombrePieza} a su posición original (X: 0, Y: 0)`}
