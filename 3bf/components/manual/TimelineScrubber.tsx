...
import { Play, Pause, RotateCcw, Volume2, VolumeX, SkipBack, SkipForward, ChevronDown, ChevronUp, Sliders, Layers, Sparkles, Smartphone, RefreshCw } from "lucide-react";
...
export default function TimelineScrubber() {
  ...
  const duracionTotal = Math.max(
    pasoActivo?.duracionTotal !== undefined && pasoActivo.duracionTotal > 0
      ? (esMultiSub3 ? Math.max(pasoActivo.duracionTotal, 13.5) : pasoActivo.duracionTotal)
      : (esMultiSub3 ? Math.max(duracionAnimCalculada, 13.5) : duracionAnimCalculada),
  ...
  const escalonarCascadaSubbloques = () => {
    ...
    const tiene3Sub = subbloques.length === 3;
    const totalSlots = tiene3Sub ? 4 : subbloques.length;
    const durSlot = Number((duracionTotal / totalSlots).toFixed(1));

    subbloques.forEach((sub, idx) => {
      let inicio = Number((idx * durSlot).toFixed(1));
      let dur = durSlot;

      if (tiene3Sub) {
        if (idx === 0) {
          // Acto 1 (Lateral 1): 0.0s -> durSlot
          inicio = 0.0;
          dur = durSlot;
        } else if (idx === 1) {
          // Acto 2 (Cara A Central): durSlot -> 2 * durSlot
          inicio = durSlot;
          dur = durSlot;
        } else if (idx === 2) {
          // Acto 3 (Lateral 2): 2 * durSlot -> 3 * durSlot (deja el 4to slot para Giro + Cara B + Tarugos)
    ...
    <div className="absolute bottom-3 left-3 right-3 z-20 flex flex-col gap-1.5 pointer-events-none select-none">
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-slate-200/70 dark:border-slate-800/70 pb-1.5">
            ...
            <div className="flex items-center gap-1.5">
              {/* Selector de Coreografía: Cascada vs Sincronizada */}
              <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-full border border-slate-200 dark:border-slate-700">
                <button
                  type="button"
                  onClick={() => {
                    setCoreografiaSubbloques(pasoActivo.id, 1);
                    despertarAnimacion();
                  }}
                  title="Coreografía 1: Cascada (corredera por corredera una a una con sus tornillos)"
                  className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold transition cursor-pointer select-none ${
                ...
                <button
                  type="button"
                  onClick={() => {
                    setCoreografiaSubbloques(pasoActivo.id, 2);
                    despertarAnimacion();
                  }}
                  title="Coreografía 2: Sincronizada (todas las correderas bajan juntas, luego todos los tornillos juntos)"
                  className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold transition cursor-pointer select-none ${
                    pasoActivo.coreografiaSubbloques === 2
                      ? "bg-[#1368AA] text-white shadow-xs"
                      : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                  }`}
                >
              ...
              {/* Botón Despertar / Recalcular Animación 3D */}
              <button
                type="button"
                onClick={despertarAnimacion}
                title="Despertar y recalcular animación 3D (resetea el AnimationMixer y sincroniza piezas)"
                className="px-2.5 py-1 rounded-full text-[9px] font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30 hover:bg-amber-500/20 transition flex items-center gap-1 cursor-pointer select-none"
              >
      ...
      >
        ...
        <button
          type="button"
          onClick={despertarAnimacion}
          title="Despertar y recalcular animación 3D (resetea mixer y reactiva cinemática)"
          className="w-7 h-7 rounded-full flex items-center justify-center border border-amber-500/40 text-amber-600 dark:text-amber-400 hover:bg-amber-500/10 transition active:scale-95"
        >
          <RefreshCw className={`w-3.5 h-3.5 ${isWaking ? "animate-spin" : ""}`} />
        </button>
