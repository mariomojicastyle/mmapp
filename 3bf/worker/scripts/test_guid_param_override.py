...
def test_guid_slider(ancho_val):
    ...
    guid_ancho = "625f42bc-be0f-47f8-a2b4-b934dea0a0d7"

    payload_rc = {
        "algo": b64_algo,
        "pointer": None,
        "values": [
            {"ParamName": guid_ancho, "InnerTree": {"{0}": [{"type": "System.Double", "data": str(float(ancho_val))}]}},
    ...
    print(f"\n--- Prueba Ancho por GUID = {ancho_val} mm ---")
    for name, x in extracted_meshes:
        print(f"  • {name} -> Tamaño X: {x:.1f} mm")

if __name__ == "__main__":
    test_guid_slider(800)
    test_guid_slider(1400)
