...
def inspect_latest_user_ghx():
    ...
    print(f"=== INSPECCIONANDO NUEVA VERSIÓN DEL ARCHIVO GHX RECIÉN GUARDADA POR EL USUARIO ===")
    ...
    print(f"\n[OK TOTAL] PIEZAS REALES EXTRAÍDAS DEL NUEVO ARCHIVO: {len(real_meshes)}")
    for m in real_meshes:
        print(f"  • Salida: '{m['name']}' (Branch: {m['path']}) | Size X: {m['size'][0]*1000:.1f}mm | Pos Y: {m['position'][1]:.3f}m")

if __name__ == "__main__":
    inspect_latest_user_ghx()
