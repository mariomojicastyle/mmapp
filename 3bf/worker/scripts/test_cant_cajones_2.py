...
def test_cajones():
    ...
    print("=== EVALUANDO RHINOCOMPUTE CON CANTIDAD DE CAJONES = 2 ===")
    payload_rc = {
        "algo": b64_algo,
        "pointer": None,
        "values": [
            ...
            {"ParamName": "RH_IN:Cantidada de Cajones", "InnerTree": {"{0}": [{"type": "System.Int32", "data": "2"}]}},
            {"ParamName": "RH_IN:Cantidad de Cajones", "InnerTree": {"{0}": [{"type": "System.Int32", "data": "2"}]}}
        ]
    }
    ...
    print(f"\n[CANTIDAD CAJONES = 2] TOTAL MASHES DEVUELTAS POR RHINO 8: {len(real_meshes)}")
    for m in real_meshes:
        print(f"  • {m['name']} (Branch: {m['path']})")
