...
async function run() {
  try {
    ...
    console.log(`2. Eliminando tabla antigua "Empresas" (ID: ${oldEmpresasTableId})...`);
    try {
      await request('DELETE', `/api/database/tables/${oldEmpresasTableId}/`, null, token);
      console.log('Tabla antigua eliminada.');
    } catch (err) {
      console.log('La tabla antigua ya no existía o falló:', err.message);
    }

    console.log('3. Creando nueva tabla "Empresas" (para que inicie en ID 1)...');
    ...
    // Primero, verificamos si existe un campo antiguo "Empresa Vinculada" en Leads para evitar duplicidad o error.
    ...
    console.log(`7. Inyectando las 15 empresas del Tier 2 a la tabla recreada (iniciando en ID 1)...`);
    for (const company of tier2Companies) {
      try {
        ...
        console.log(`   Inyectada: ${company["Nombre de la Empresa"]} (Nueva fila ID: ${response.id})`);
      } catch (err) {
        console.error(`   Error al inyectar ${company["Nombre de la Empresa"]}:`, err.message);
      }
    }

    console.log(`\n¡Tabla Empresas recreada y repoblada con éxito! Nueva ID de Tabla: ${empresasTableId}`);
  } catch (error) {
