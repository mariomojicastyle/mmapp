...
* **[2026-07-23] Visor 3D — Corrección de Navegación del Botón Home y Detención de Audio:**
    - **Pausa Inmediata de Audio**: Al hacer clic en el botón Home ("casita"), se invoca `useEnviroment.getState().PausedAudio()` para cortar la reproducción de audio al instante ([RealidadAumentada.jsx](file:///c:/Desarrollo/mmapp/legacy-aplicativo-armado/src/features/AssemblyInstructions/components/NavBarSuperior/RealidadAumentada/RealidadAumentada.jsx)).
    - **Navegación en la Misma Pestaña**: Reemplazado `window.open(..., '_blank')` por `window.top.location.href`, permitiendo al usuario retornar a la página principal en la misma pestaña sin dejar el manual sonando en segundo plano ni abrir duplicados.
