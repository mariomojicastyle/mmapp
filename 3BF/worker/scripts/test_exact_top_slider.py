...
def test_top_slider_only():
    print("=== Configurando ÚNICAMENTE el Slider Principal Ancho (SourceCount: 0) ===")
    ...
    top_ancho_guid = "625f42bc-be0f-47f8-a2b4-b934dea0a0d7"

    # 1. Cambiar ÚNICAMENTE el NickName del slider principal
    for item in root.iter("chunk"):
        guid_elem = item.find("./items/item[@name='InstanceGuid']")
        if guid_elem is not None and guid_elem.text == top_ancho_guid:
            for sub in item.iter("item"):
                if sub.attrib.get("name") == "NickName" and sub.text:
                    sub.text = "RH_IN:Ancho"
                    print("  * NickName del Slider Principal Ancho actualizado a 'RH_IN:Ancho'")

    # 2. Asignar RH_OUT a las salidas preferidas TapaLuz y Frente de Cajon
    for item in root.iter("chunk"):
    ...
    # Evaluacion con RH_IN:Ancho = 600mm vs 900mm vs 400mm
    def test_val(val):
        payload_rc = {
            "algo": b64_algo,
            "pointer": None,
            "values": [
                ...
                {"ParamName": top_ancho_guid, "InnerTree": {"{0}": [{"type": "System.Double", "data": str(float(val))}]}}
            ]
        }
