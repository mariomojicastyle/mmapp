...
def fix_all_inputs_and_outputs(input_ghx, output_ghx):
    print("=== Aplicando Etiquetado Completo en Sliders, Componentes y Grupos ===")
    tree = ET.parse(input_ghx)
    root = tree.getroot()
    
    slider_targets = ["Ancho", "Alto", "Profundidad", "Cantidada de Cajones", "Abrir Cajones"]
    ...
    for item in root.iter("chunk"):
        ...
        if comp_name == "Number Slider":
            for st in slider_targets:
                if (st in comp_nick or st == comp_name) and not comp_nick.startswith("RH_IN:"):
                    if nick_elem is not None:
                        nick_elem.text = f"RH_IN:{st}"
                        print(f"  * Slider NickName actualizado a: 'RH_IN:{st}'")
    ...
    for item in root.iter("chunk"):
        ...
        if guid_item is not None and guid_item.text:
            ...
            if name == "Number Slider" or any(st in nick for st in slider_targets):
                clean_name = nick.replace("RH_IN:", "")
                slider_guids[guid] = clean_name
                ...
                if items_block is not None:
                    ...
                    for g_guid, g_name in geom_guids.items():
                        if g_guid in contained:
                            target_name = f"RH_OUT:{g_name.replace('RH_OUT:', '')}"
    ...
    print("\n--- Respuesta Final Diagnostico RhinoCompute /io ---")
...
if __name__ == "__main__":
    ...
    fix_all_inputs_and_outputs(src, dst)
