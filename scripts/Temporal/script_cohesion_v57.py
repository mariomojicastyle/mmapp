...
def es_herraje(nombre_base):
    ...
    # Si el nombre es puramente numérico (ej. "20020006"), asumimos que es un SKU de herraje
    if re.fullmatch(r'\d+', nombre_base.strip()):
        return True
...
def comparten_keyword_herraje(n1, n2):
    ...
    # Si uno es puramente un SKU numérico y está contenido en el otro, comparten familia
    sku_n1 = re.fullmatch(r'\d+', n1.strip())
    sku_n2 = re.fullmatch(r'\d+', n2.strip())
    
    if sku_n1 and n1.strip() in n2:
        return True
    if sku_n2 and n2.strip() in n1:
        return True
...
def determinar_nombre_base(nombres_en_grupo):
    ...
    if candidatos:
        # Para láminas preferimos el más largo para no perder la descripción principal (ej. "Paral 5A" frente a "5A")
...
def ejecutar_cohesion():
    ...
    for idx, grupo in enumerate(grupos_finales):
        ...
        if not es_identica and not modo_herraje:
            # Según regla estricta: ignoramos solamente lo que va después del guión "-"
            if "-" in nombre_base:
    ...
    # Ordenamos los nombres base para que las piezas que sean "fondo" se procesen de últimas
    nombres_ordenados = sorted(piezas_finales_dict.keys(), key=lambda n: 1 if "fondo" in n.lower() else 0)
