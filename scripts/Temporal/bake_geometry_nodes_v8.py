...
def bake_geometry_nodes_instances(emitter_prefix="Plane", start_frame=None, end_frame=None, export_glb=True, revert_blend=True):
    ...
    V8 - Soluciona el problema de "GLB invisible". Desoculta los objetos originales (ícono del ojo) 
         antes de exportar. Los objetos originales suelen estar ocultos para no interferir con 
         Geometry Nodes, pero si se exportan ocultos, el GLB los interpreta como invisibles.
    """
    ...
    print(f"\n==================================================")
    print(f"INICIANDO BAKE DIRECTO Y EXPORTACIÓN GLB (V8)")
    ...
    try:
        ...
        # --- FASE 4.5: DESOCULTAR OBJETOS (V8 FIX) ---
        # Como los objetos originales suelen estar ocultos para no interferir con Geometry Nodes,
        # debemos desocultarlos antes de exportar. De lo contrario, el GLB los exporta como "invisibles".
        print(" -> Desocultando objetos originales para que sean visibles en el GLB...")
        for orig_obj in bake_data.keys():
            try:
                orig_obj.hide_set(False)      # Ícono del ojo (Viewport)
                orig_obj.hide_viewport = False # Monitor (Viewport global)
                orig_obj.hide_render = False   # Cámara (Render)
            except Exception:
                pass
    ...
    print(f"\n==================================================")
    print(f"BAKE Y EXPORTACIÓN COMPLETADOS EXITOSAMENTE (V8)")
    print(f"El archivo .blend quedó intacto y se generó el GLB correctamente.")
    print(f"==================================================\n")
