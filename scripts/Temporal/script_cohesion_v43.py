...
def herrajes_comparten_eje(obj1, obj2, margin_x=0.12, margin_z=0.12):
    c1 = get_mesh_center(obj1)
    c2 = get_mesh_center(obj2)
    if not c1 or not c2:
        return False
    ...
    # Si comparten X y Z (están en el mismo lado del mismo cajón)
    # Permite variación en Y (por ej. si el cajón se modeló abierto)
    if dx <= margin_x and dz <= margin_z:
        return True
...
def ejecutar_cohesion():
    ...
    while herrajes_iniciales:
        ...
        agregado_nuevo = True
        while agregado_nuevo:
            agregado_nuevo = False
            vecinos_a_remover = []
            
            for otra in mallas_pendientes:
                nombre_otra_base = otra.name.split('.')[0].strip()
                nombre_otra_lower = nombre_otra_base.lower()
                
                # Criterios de compatibilidad para agrupar herrajes:
                # 1. Empieza por mesh (es huérfana)
                # 2. Tienen el mismo nombre base exacto
                # 3. Comparten el mismo keyword de herraje (ej. "corredera" en ambos)
                es_compatible = (
                    nombre_otra_lower.startswith("mesh") or 
                    nombre_otra_base == nombre_base_actual or 
                    comparten_keyword_herraje(nombre_base_actual, nombre_otra_base)
                )
                
                # Si es una pieza que se debe animar por separado, cancelamos la compatibilidad
                if es_compatible and (no_agrupar_nunca(nombre_base_actual) or no_agrupar_nunca(nombre_otra_base)):
                    es_compatible = False
                
                if es_compatible:
                    # Chequear solapamiento contra CUALQUIER miembro actual del grupo (Transitivo)
                    if comparten_keyword_herraje(nombre_base_actual, nombre_otra_base):
                        # Para herrajes de la misma familia, si el cajón está abierto pueden estar separados en Y.
                        # Chequeamos solapamiento físico OR si están alineados en sus ejes de montaje.
                        toca = any(bbox_overlap(miembro, otra, margin=0.1) or herrajes_comparten_eje(miembro, otra) for miembro in grupo)
                    else:
                        toca = any(bbox_overlap(miembro, otra, margin=0.02) for miembro in grupo)
