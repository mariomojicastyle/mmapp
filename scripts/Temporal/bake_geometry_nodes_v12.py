...
def bake_geometry_nodes_instances(emitter_prefix="Plane", start_frame=None, end_frame=None, export_glb=True, revert_blend=True):
    """
    V12 (Mecánica Orbital Verdadera)
    - Soluciona el colapso al rotar piezas escaladas.
    - El Motor Orbital ahora extrae la Macro-Transformación perfecta. En lugar de copiar
      ciegamente a la pieza principal, calcula matemáticamente el PIVOTE EXACTO de tu 
      Geometry Nodes.
    - Resultado: Las curvas de animación local son 100% planas y rígidas. Cero bamboleo 
      (wobble) de interpolación al exportar a GLB, sin importar giros agresivos o escalas.
    """
    ...
    print(f"\n==================================================")
    print(f"INICIANDO BAKE Y EXPORTACIÓN GLB (V12 INMACULADA)")
    print(f"==================================================")
    ...
    try:
        ...
        # Buscar la matriz de referencia (cuando la pieza principal es normal)
        W_ref = None
        if mueble_principal:
        ...
        # Aislar las verdaderas coordenadas locales de cada pieza respecto a la transformación nula
        child_refs = {}
        ...
        print("\nFase 4: Horneando keyframes de Motor Macro y estabilizando locales...")
        for frame in range(start_frame, end_frame + 1):
            scene.frame_set(frame)
            ...
            if mueble_principal:
                ...
                if mueble_frame_data is not None:
                    # EXTRAER MACRO-TRANSFORMACIÓN PURA (sin el offset de la pieza)
    ...
    print(f"\n==================================================")
    print(f"V12 INMACULADA - FINALIZADA")
    print(f"==================================================\n")
