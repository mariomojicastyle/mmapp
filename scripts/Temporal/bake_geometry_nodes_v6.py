...
def bake_geometry_nodes_instances(emitter_prefix="Plane", start_frame=None, end_frame=None, export_glb=True, revert_blend=True):
    ...
    V6 Corregida - Resuelve la colisión e intersección oblicua de fondos y puntillas después del frame 48 (giro)
                   y blinda el script contra errores de exportación y pérdida de datos usando try-finally.
    """
    ...
    print(f"\n==================================================")
    print(f"INICIANDO BAKE DIRECTO Y EXPORTACIÓN GLB (V6 CORREGIDA)")
    ...
    # Estructura para almacenar los Empties temporales creados y poder referenciarlos
    empty_objs = []
    
    # Usamos try-finally para garantizar que el archivo .blend se revierta pase lo que pase (evita viewport vacío si el script falla)
    try:
        ...
        if not mueble_principal:
            # Fallback: buscar la pieza más grande que no sea puntilla, fondo ni herraje
            for obj in bake_data.keys():
        ...
        for orig_obj in list(bake_data.keys()):
            ...
            # Si no es el mueble principal, creamos su Empty intermedio para heredar rotación y mantener el nombre
        ...
        for frame in range(start_frame, end_frame + 1):
            ...
            # B. Forzar la actualización de la escena para que los Empties temporales asuman su pose correcta en el mundo
            bpy.context.view_layer.update()
            
            # C. Procesar el resto de objetos hijos usando las matrices actualizadas de los Empties
            for orig_obj, keyframes in bake_data.items():
                ...
                if orig_obj == mueble_principal:
                    ...
                    # Si tiene padre (que es el Empty), obtenemos la matriz del mundo actualizada del Empty
                    if orig_obj.parent:
        ...
        # D. Configurar interpolación LINEAL en todas las F-Curves horneadas para evitar desalineaciones por ease Bézier
        ...
        if export_glb:
            ...
            if not blend_filepath:
                ...
                try:
                    # Usamos los parámetros de exportación tradicionales que son 100% compatibles en cualquier versión de Blender
                    ...
                    # Si falla, relanzamos la excepción para asegurarnos de que el finally la capture
                    raise e
                
    finally:
        # --- FASE 6: REVERTIR ARCHIVO .BLEND (OBLIGATORIO) ---
        # El bloque finally garantiza que la escena original de Blender se restaure intacta
        # incluso si el script de Python arroja algún error durante el bake o la exportación GLB.
        if revert_blend:
            ...
            # Si el usuario explícitamente desactivó revertir (para depuración), restauramos la pose original
    ...
    print(f"\n==================================================")
    print(f"BAKE Y EXPORTACIÓN COMPLETADOS EXITOSAMENTE (V6)")
