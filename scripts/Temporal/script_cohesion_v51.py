...
def should_group(actual, otra):
    ...
    # REGLA 2: Por Contención (con margen ampliado)
    # Un margen de 4cm permite que cantos, molduras o placas dobles (que suelen medir 15mm-18mm)
    # se reconozcan como "contenidos" dentro de la placa principal, sin fusionar paredes perpendiculares.
    if n1 == n2:
        if bbox_contains(actual, otra, margin=0.04) or bbox_contains(otra, actual, margin=0.04):
            return True
    ...
    # CRÍTICO: Usamos un margen ultra preciso de 1mm (0.001m) en lugar de 2cm (0.02m).
    # Esto es vital para NO saltar la luz/gap de 3mm entre cajones o puertas contiguas, 
    # evitando que dos frentes de cajón distintos se fusionen en uno solo.
    if (n1 == n2 and es_nombre_principal(n1)) or n1_lower.startswith("mesh") or n2_lower.startswith("mesh"):
        if bbox_overlap(actual, otra, margin=0.001):
            return True
