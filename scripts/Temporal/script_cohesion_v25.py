...
def ejecutar_cohesion():
    print("\n--- INICIANDO COHESIÓN ESPACIAL V25 (PARCHES EXCLUSIVOS PARA MADERA) ---")
    ...
    # ---------------------------------------------------------
    # FASE 3: ABSORCIÓN DE RECORTES Y PARCHES 2D
    # ---------------------------------------------------------
    grupos_finales = []
    huerfanos = []
    ...
    for h in huerfanos:
        ...
        for g in grupos_finales:
            principal = g[0]
            nombre_principal = principal.name.split('.')[0]
            
            # ¡REGLA DE ORO! Los herrajes (Correderas, Bisagras, etc.) 
            # JAMÁS pueden absorber parches de madera.
