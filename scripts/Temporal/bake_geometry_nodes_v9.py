...
def bake_geometry_nodes_instances(emitter_prefix="Plane", start_frame=None, end_frame=None, export_glb=True, revert_blend=True):
    """
    V9 INMACULADA
    Arquitectura reescrita de jerarquías. Desacopla la rotación/posición de la escala 
    mediante un 'Motor Orbital' que garantiza cero distorsiones, cero crashes por matrices
    y preserva las animaciones de escala independientes de todas las piezas.
    """
    ...
    print(f"\n==================================================")
    print(f"INICIANDO BAKE Y EXPORTACIÓN GLB (V9 INMACULADA)")
    print(f"==================================================")
    ...
    try:
        ...
        # Crear el Motor Orbital Global (Siempre tendrá escala 1,1,1)
        ...
        for orig_obj in list(bake_data.keys()):
            ...
            if clean_name not in created_empties:
                ...
                # Todo se ancla al Motor Orbital Global
        ...
        for frame in range(start_frame, end_frame + 1):
            ...
            # El motor orbital copia la posición y rotación del mueble principal, PERO NUNCA SU ESCALA
            if mueble_principal:
            ...
            for orig_obj, keyframes in bake_data.items():
                ...
                try:
                    ...
                    # Defensa extrema, aunque con el motor orbital la escala 0 ya no existe en la jerarquía
