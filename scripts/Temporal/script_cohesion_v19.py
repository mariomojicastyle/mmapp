...
def forman_paralelepipedo(obj1, obj2, tolerancia=0.01):
    """
    NORMA DEL PARALELEPÍPEDO:
    Calcula la caja delimitadora (bounding box) de la unión de ambas piezas.
    Si las dos piezas al unirse siguen manteniendo el largo y el ancho de la pieza original,
    entonces forman un paralelepípedo perfecto (ej. Puerta + Círculo = Puerta).
    Si al unirse se ensanchan creando una forma de "L" (ej. Frente + Lateral), la prueba falla.
    """
    ...
    def match_2_largest(d_main, d_test):
        return (abs(d_main[1] - d_test[1]) <= tolerancia and 
                abs(d_main[2] - d_test[2]) <= tolerancia)
...
def should_group(actual, otra):
    ...
    # REGLA 1: Por distancia de Centros (Idéntica a V17)
    ...
    # REGLA 2: Norma del Paralelepípedo
    # Para atrapar recortes como el círculo de MDP.
    # Exigimos que se toquen y que al fusionarse sigan manteniendo la forma de bloque rectangular.
    if bbox_overlap(actual, otra, margin=0.005):
        if forman_paralelepipedo(actual, otra, tolerancia=0.01):
            return True
...
def ejecutar_cohesion():
    print("\n--- INICIANDO COHESIÓN ESPACIAL V19 (V17 + NORMA DE PARALELEPÍPEDO) ---")
    ...
    # ---------------------------------------------------------
    # FASE 1: RECONSTRUCCIÓN DE HERRAJES (V17)
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # FASE 2: COHESIÓN DE LÁMINAS DE MADERA (V17 + Paralelepípedo)
    # ---------------------------------------------------------
