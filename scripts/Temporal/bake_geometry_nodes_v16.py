...
def bake_geometry_nodes_instances(emitter_prefix="Plane", start_frame=None, end_frame=None, export_glb=True, revert_blend=True):
    """
    V16 (La Versión Definitiva)
    - Basada en la V14 que sí exportaba las mallas, pero con el fix de herencia de escala de la V15.
    - Se descubrió que el Exportador GLTF de Blender tiene un BUG INTERNO: Si el padre (Motor Orbital) 
      tiene una escala exacta de 0 en el fotograma de exportación, el exportador colapsa y elimina toda 
      la jerarquía (dejando el GLB vacío).
    - V16 hereda la escala no-uniforme para evitar que las piezas roten 90 grados (Cizalladura),
      pero le pone un CLAMP mínimo de 0.001. Esto engaña al exportador GLTF para que exporte todo,
      y mantiene la matemática pura.
    """
    ...
    print(f"\n==================================================")
    print(f"INICIANDO BAKE Y EXPORTACIÓN GLB (V16)")
    print(f"==================================================")
    ...
    try:
        ...
        print("\nFase 3: Construyendo Motor Orbital (V16 Core)...")
        ...
        print("\nFase 4: Horneando keyframes con Fix Definitivo Anti-Vacio y Anti-Cizalladura...")
        for frame in range(start_frame, end_frame + 1):
            scene.frame_set(frame)
            ...
            if orbital_motor_ref:
                ...
                if mueble_frame_data is not None:
                    ...
                    # V16 FIX CORE: 
                    # Heredamos la escala (sca) para evitar Cizalladura (piezas rotando a 90 grados en V14).
                    # PERO le ponemos un mínimo de 0.001. El exportador de GLTF de Blender tiene un BUG
                    # donde si un nodo padre tiene escala exacta 0, descarta toda la rama (causando GLB vacío).
                    safe_sca = mathutils.Vector((
                        sca.x if abs(sca.x) >= 0.001 else (0.001 if sca.x >= 0 else -0.001),
                        sca.y if abs(sca.y) >= 0.001 else (0.001 if sca.y >= 0 else -0.001),
                        sca.z if abs(sca.z) >= 0.001 else (0.001 if sca.z >= 0 else -0.001)
                    ))
                    
                    orbital_motor.matrix_world = mathutils.Matrix.LocRotScale(loc, rot, safe_sca)
                    orbital_motor.location = loc
                    orbital_motor.rotation_quaternion = rot
                    orbital_motor.scale = safe_sca
    ...
    print(f"\n==================================================")
    print(f"V16 - FINALIZADA (LA VERSIÓN DEFINITIVA)")
    print(f"==================================================\n")
