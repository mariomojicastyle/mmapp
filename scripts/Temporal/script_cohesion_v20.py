...
def forman_paralelepipedo(obj1, obj2):
    ...
    Corrige el error de las uniones en "L" o "T" evaluando el grosor final.
    """
    dim_comb = bbox_combined_dimensions(obj1, obj2)
    dim1 = tuple(sorted(obj1.dimensions))
    dim2 = tuple(sorted(obj2.dimensions))
    
    # 1. EVITAR FUSIÓN PERPENDICULAR (La pieza final no puede engordar)
    # Si unes un Frente y un Lateral, la "caja virtual" resultante tendrá un grosor enorme (aprox 50cm).
    max_grosor_original = max(dim1[0], dim2[0])
    if dim_comb[0] > max_grosor_original + 0.025: # Tolerancia de 2.5cm
        return False
...
def should_group(actual, otra):
    ...
    # REGLA 2: "Norma del Paralelepípedo" con búsqueda de Contacto
    # Usamos un margen de 3cm por si el círculo está físicamente ligeramente separado de la puerta.
...
def ejecutar_cohesion():
    print("\n--- INICIANDO COHESIÓN ESPACIAL V20 (PARALELEPÍPEDO BLINDADO) ---")
