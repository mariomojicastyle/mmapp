...
def ejecutar_cohesion():
    print("\n--- INICIANDO COHESIÓN ESPACIAL V28 (NORMALES CORREGIDAS + MARGEN DINÁMICO) ---")
    ...
    for h in huerfanos:
        ...
        for g in grupos_finales:
            ...
            # Si el parche tiene exactamente el mismo nombre base que la madera, somos generosos (2cm)
            # Si tienen nombre distinto (ej. MDP anónimo), somos súper estrictos (5mm)
            margen_usado = 0.02 if nombre_principal == nombre_base_h else 0.005
            
            if bbox_contains(principal, h, margin=margen_usado):
    ...
    # ---------------------------------------------------------
    # FASE 4: FUSIÓN, NORMALES Y NOMENCLATURA
    # ---------------------------------------------------------
    piezas_finales_dict = {}
    ...
    for idx, grupo in enumerate(grupos_finales):
        ...
        # 1. Ajustar el origen al centroide
        bpy.ops.object.origin_set(type='ORIGIN_GEOMETRY', center='BOUNDS')
        
        # 2. Recalcular las Normales hacia afuera (Soluciona piezas invisibles / oscuras)
        if obj_unido.type == 'MESH':
