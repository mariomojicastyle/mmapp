        ...
        while agregado_nuevo:
            ...
            for otra in mallas_pendientes:
                ...
                if es_compatible:
                    ...
                    # Usamos un margen enorme (10cm) para piezas que comparten la misma familia (ej. "corredera")
                    # para garantizar que todas las partes telescópicas se fusionen.
                    margen_overlap = 0.1 if comparten_keyword_herraje(nombre_base_actual, nombre_otra_base) else 0.02
                    toca = any(bbox_overlap(miembro, otra, margin=margen_overlap) for miembro in grupo)
                    if toca:
                        grupo.append(otra)
