...
def bake_geometry_nodes_instances(emitter_prefix="Plane", start_frame=None, end_frame=None, export_glb=True, revert_blend=True):
    """
    V11 (La Inmaculada y Definitiva)
    - Elimina por completo las jerarquías complejas y los Empties redundantes.
    - Ancla todo directamente al Motor Orbital con 'Matrix_Parent_Inverse' en identidad pura.
    - Implementa "Fase 1.5: Anti-GiroLoco": Detecta cuando Geometry Nodes escala objetos a 0. 
      Las matrices con escala 0 pierden su rotación (basura matemática). Al escalar a >0, 
      la pieza saltaba violentamente a su rotación real ("colapso y corrección"). 
      Ahora escanea el futuro/pasado para inyectar la rotación correcta a los frames invisibles.
    """
    ...
    print(f"\n==================================================")
    print(f"INICIANDO BAKE Y EXPORTACIÓN GLB (V11 INMACULADA)")
    print(f"==================================================")
    ...
    try:
        ...
        # FASE 1.5: ANTI-GIRO LOCO (ESTABILIZACIÓN DE ESCALA CERO)
        ...
        for orig_obj, keyframes in bake_data.items():
            for i in range(len(keyframes)):
                ...
                # Si la escala es casi cero, la rotación matemática es basura.
                if s.length <= 0.001:
                    closest_rot = r
                    min_dist = 999999
                    # Buscar la rotación válida más cercana en el tiempo
                    ...
                    # Reescribimos la matriz con la rotación real para que cuando crezca, no gire salvajemente
        ...
        # Emparentado directo sin empties intermedios
        for orig_obj in list(bake_data.keys()):
            ...
            orig_obj.matrix_parent_inverse = mathutils.Matrix() # Identidad perfecta
    ...
    print(f"\n==================================================")
    print(f"V11 INMACULADA - FINALIZADA")
    print(f"==================================================\n")
