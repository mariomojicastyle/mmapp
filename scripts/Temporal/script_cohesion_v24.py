...
def ejecutar_cohesion():
    print("\n--- INICIANDO COHESIÓN ESPACIAL V24 (ESTABILIDAD TOTAL + ABSORCIÓN 2D) ---")
    ...
    # ---------------------------------------------------------
    # FASE 3: ABSORCIÓN DE RECORTES Y PARCHES 2D (EL CÍRCULO MDP Y TAPAS DE RANURA)
    # ---------------------------------------------------------
    grupos_finales = []
    huerfanos = []
    ...
    for g in grupos_fisicos:
        ...
        es_parche_2d = dims_g[0] <= 0.002 # Menor o igual a 2 milímetros (incluye piezas 2D de 0m)
        
        # Un huérfano es: Una pieza suelta que O bien no tiene nombre principal, O bien es un parche sin grosor
    ...
    for h in huerfanos:
        ...
        for g in grupos_finales:
            principal = g[0]
            # Usamos un margen de 1.5cm para asegurar que si el parche está al ras de la cara lo absorba
