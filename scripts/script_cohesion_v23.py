...
def forman_paralelepipedo(obj1, obj2):
    ...
    # Tolerancia estricta de grosor: La pieza final no puede engordar más de 5 milímetros.
...
def ejecutar_cohesion():
    print("\n--- INICIANDO COHESIÓN ESPACIAL V23 (CENTROS + GEOMETRÍA BLINDADA) ---")
    ...
    while mallas_pendientes:
        ...
        for otra in mallas_pendientes:
            ...
            # 1. Tienen que compartir el mismo centro físico (como la Cara y el MDP original de Grasshopper)
            if c1 and c2 and (c1 - c2).length <= UMBRAL_CENTROS:
                # 2. En vez de bloquearlos si se llaman distinto (ej. "MDF_fondo" y "Fondo"), 
                # validamos matemáticamente que al unirse mantengan el grosor y tamaño original de la lámina.
