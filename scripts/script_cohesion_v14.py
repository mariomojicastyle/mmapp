...
def comparten_dos_dimensiones(obj1, obj2, tolerancia=0.005):
    """
    Verifica si dos objetos comparten al menos 2 de sus 3 dimensiones (X, Y, Z).
    Esto garantiza que piezas como Cara y MDP (que comparten Largo y Ancho, 
    pero difieren en Grosor) se agrupen, mientras que piezas irregulares 
    como Caja y Perno (que no comparten dimensiones) sean rechazadas.
    """
    d1 = obj1.dimensions
    d2 = obj2.dimensions
    
    coincidencias = 0
    if abs(d1.x - d2.x) <= tolerancia: coincidencias += 1
    if abs(d1.y - d2.y) <= tolerancia: coincidencias += 1
    if abs(d1.z - d2.z) <= tolerancia: coincidencias += 1
...
def ejecutar_cohesion():
    print("\n--- INICIANDO COHESIÓN ESPACIAL (PASO 2 CORREGIDO V14) ---")
    ...
    # AGRUPAMIENTO GEOMÉTRICO (V12 ESTABLE + REGLA DE PARALELEPÍPEDO)
    grupos_fisicos = []
    ...
    while mallas_pendientes:
        ...
        for otra in mallas_pendientes:
            ...
            if centro_actual and centro_otra:
                ...
                # REGLA 1: Deben estar físicamente cerca (Umbral V12)
                if distancia <= UMBRAL_CENTROS:
                    # REGLA 2: (NORMA DEL PARALELEPÍPEDO) Deben compartir 2 dimensiones (Largo y Ancho)
                    if comparten_dos_dimensiones(actual, otra):
                        grupo.append(otra)
    ...
    print("\n✅ SCRIPT V14 (REGLA PARALELEPÍPEDO) FINALIZADO.")
    bpy.ops.object.select_all(action='DESELECT')
