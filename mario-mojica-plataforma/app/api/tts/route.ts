...
async function synthesizeTtsWithPauses(text: string, voice: string): Promise<Buffer> {
  ...
  if (hasPauses) {
    ...
    const cleanSilenceBuffer = stripId3(silenceBuffer)
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (segment.type === "text" && typeof segment.value === "string") {
        ...
        // Keep ID3 only for the very first segment, strip it from subsequent segments
        const finalBuf = (i === 0) ? buf : stripId3(buf)
