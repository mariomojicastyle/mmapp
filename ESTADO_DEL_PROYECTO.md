...
- [x] **Perfeccionamiento Milimétrico de Nubes de Ayuda en Móvil (17 de Junio, 2026)**:
  - **Barra Superior**: Fijada la altura de `.contenedor1` a exactamente `52px !important` en móviles para estabilizar la posición vertical de los botones. Ajustado el `top` de las burbujas superiores a `56px !important` para dejar exactamente 2 píxeles de luz de holgura entre la flecha y los botones de Guía, Iluminación, Velocidad e Idioma.
  - **Barra Inferior**: Re-alineadas las burbujas inferiores reduciendo su altura para acoplarse directamente a los botones táctiles: `ayuda3` (Navegación) centrada a `bottom: 60px !important` (2px del círculo central de pasos de 52px), `ayuda4` (Buscador) a la izquierda a `bottom: 56px !important` (2px de la lupa), y `ayuda5` (Play/Pausa) a la derecha a `bottom: 56px !important` (2px del botón de play), todas con flechas ajustadas a sus respectivos offsets y rotaciones.
